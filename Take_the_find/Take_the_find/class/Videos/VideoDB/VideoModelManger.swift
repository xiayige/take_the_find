...
/**表名*/
private let TABLENAME = "videoM"
class VideoModelManger: NSObject {
   
    /**数据库指针*/
    let FMDB:FMDatabase
    static let manger = VideoModelManger()
    override init() {
        //数据库路径 path
        ...
        //创建表
        let creatSq = "create table if not exists %@(id varchar(256) primary key ,str varchar(256))"
    ...
    /**插入语句*/
    func insertSql(id:String,str:NSString){
       
        let insertSq = "insert into %@(id, str) values(?, ?)"
        let insertSql = String.init(format: insertSq, TABLENAME)
        do {
            try FMDB.executeUpdate(insertSql, values: [id,str])
            print("数据插入成功")
        } catch {
            print("数据插入失败-\(FMDB.lastErrorMessage())")
        }
    }
    /**从userInfo里查询所有数据--根据模型取值取出来直接变为数组*/
    func searchSql() -> NSString?{
        let selectSq = "select * from %@"
        var dictstr = ""
        let selectSql = String.init(format: selectSq, TABLENAME)
        do {
            ...
            if dictstr == ""{
                ...
                return nil
                 ...
                 print("查询所有数据语句成功--)")
                return dictstr as NSString
            }
        } catch {
            print("查询所有数据语句失败-\(FMDB.lastErrorMessage())")
             return nil
        }
